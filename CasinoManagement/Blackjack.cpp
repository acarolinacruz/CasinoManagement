...
Return:
   void
================================================== */
void BLACKJACK::chooseLevelOfDificulty() {
    ...
    if (level != 1) {
        ...
        if (level != 2) {
            ...
            cout << "Please enter your level of difficulty using the number 1 or 2."
                << endl;
...
Return:
   void
================================================== */
void BLACKJACK::shuffle(int deck[], int size) {
...
Return:
   void
================================================== */
void BLACKJACK::ShowCard(int card) {
...
Return:
   void
================================================== */
void BLACKJACK::showCards(const int deck[], int numCards, bool hideFirstCard) {
...
Return:
   void
================================================== */
void BLACKJACK::dumpDeck(int deck[], int size) {
...
//================================================================
int BLACKJACK::getRandomNumber(int low, int high) {
...
Return:
   int - The given Card's Value.
================================================== */
int BLACKJACK::getTopCard(int deck[]) {
...
Return:
   void
================================================== */
void BLACKJACK::addToHand(int hand[], int cardToAdd) {
...
Return:
   void
================================================== */
void BLACKJACK::hitUntilStand(int dHand[], int deck[], int pHand[]) {
        ...
        if( level == 2)
        {
          ...
          for( int i = 0; i < 9; i++)
          {
            ...
            if(CardValue(dHand[i]) == 11)
            {
              ...
              dHand[i] = dHand[i] + 13; // A hard Ace (worth 1 point) has a
        background value ending in 14
            }
          }
        }
...
================================================== */
int BLACKJACK::getHandValue(const int hand[]) {
    // Declare local variables
    int addCardValues = 0;
...
// draw lines
// PASS THIS FUNCTION TO UTILSSSSS URGENTTTTTT!!!!!!!!!!!!
void BLACKJACK::drawLine(int n, char symbol) {
    for (int i = 0; i < n; i++)
        cout << symbol;
    cout << "\n";
}
...
==================================================*/
bool BLACKJACK::playAnotherHand(char& Play) {
...
//=================================================================
bool BLACKJACK::userWantsToDraw(char& Draw) {
...
Return:
   void
================================================== */
void BLACKJACK::whoWins(const int pHand[], const int dHand[]) {
    // Declare local variables
...
Return:
   void
================================================== */
void BLACKJACK::checkBust(const int pHand[], const int dHand[], char Play) {
    // Define local variables.
    int playerScore = getHandValue(pHand);
    int dealerScore = getHandValue(dHand);
...
Return:
   void
================================================== */
void BLACKJACK::backdoorKenny(const int hand[]) {
    // If index 0 is a 10 and index 1 is an ace
    if ((CardValue(hand[0]) % 100 == 10) &&
        (CardValue(hand[1]) % 100 ==
            11)) // I did not use 1 as an argument because 11 is the default
...
Return:
   void
================================================== */
void BLACKJACK::blackJack(const int pHand[], const int dHand[], char Play) {
    // Define local Variables
    int playerScore = getHandValue(pHand);
    int dealerScore = getHandValue(dHand);
    ...
    if ((playerScore == 21) && (dealerScore != 21)) {
        // Display message, compute new winnings multiplier, ask to play another
        // hand.
        cout << "\n\n";
        scoreBoard(pHand, dHand);
        cout << "\n";
    ...
    // If both Player and Dealer have blackjack.
        ...
        scoreBoard(pHand, dHand);
        cout << "\n";
        cout << "The Dealer and you both got Blackjack. Push in your favor at 1:1 "
            "payout!"
            << endl;
        win++;
        cout << "\n";
...
Return:
   void
================================================== */
void BLACKJACK::naturalBlackJack(const int pHand[], const int dHand[],
    char Play) {
    // Define local variables
    int playerScore = getHandValue(pHand);
    int dealerScore = getHandValue(dHand);
...
Return:
   void
================================================== */
void BLACKJACK::scoreBoard(const int pHand[], const int dHand[]) {
...
Return:
   void
================================================== */
void BLACKJACK::checkSoftOrHard(int pHand[]) {
    // check cards in hand
    for (int i = 0; i < 9; i++) {
        // Define local variables.
        ...
        if (checkAce == 1 || checkAce == 11) {
            ...
            {
                // Clears input error flags and removes everything currently in the
                // input buffer.
                cin.clear();
                cin.ignore(numeric_limits<streamsize>::max(), '\n');
...
Return:
   void
================================================== */
void BLACKJACK::softOrHardAI(int dHand[], int pHand[]) {
    // check cards in hand
    for (int i = 0; i < 9; i++) {
        // Find an Ace
        if (CardValue(dHand[i]) == 1 || CardValue(dHand[i]) == 11) {
            ...
            if (CardValue(dHand[i]) == 11) {
                ...
                if (getHandValue(dHand) - CardValue(dHand[i]) + 1 >
                    getHandValue(pHand)) {
                    //...and not bust.
            ...
            // Else Ace is worth 1 point.
                ...
                if (getHandValue(dHand) - CardValue(dHand[i]) + 11 >
                    getHandValue(pHand)) {
                    //...and not bust.
